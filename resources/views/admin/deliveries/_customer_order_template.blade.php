...
<tr class="{{ $order->id }}">
    ...
    @if(!empty($order['customer']))
        <td class="width-table-name">
                <a class="decoration-hover" onclick="event.stopPropagation();" href="{{route('admin.customer.show', $order['customer']->id)}}">
                    {{ $order['customer']->name }}
                </a><br>
    ...
    <td class="text-right">
        {{ CommonHelper::commonCurrency($total) }}</td>
    <td>
    ...
    <td>
        <button class="btn btn-primary confirm-order submit-button" data-type="edit" data-id= "{{ $order->id }}" data-href=" {{ route('admin.deliveries.edit') }}">{{ trans('delivery.list.confirm') }}</button>
        <button class="btn btn-danger delete-order" data-order_id= "{{ $order->id }}" data-href=" {{ route('admin.deliveries.cancel',['status' => 0]) }} ">{{ trans('delivery.list.cacel') }}</button>
    </td>
</tr>